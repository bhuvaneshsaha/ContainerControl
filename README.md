...
- Angular: sign-in, permissions, applications, secrets, hosts, capacity, domains, access, and tokens. Nav and route guards use permission codes. The Access page is available to `access.users.manage`, `access.teams.manage`, `access.roles.manage`, or `access.audit.read`. The Capacity page is available to `platform.quotas.manage` or `platform.capacity.read`.
