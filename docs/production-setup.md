...
4. Install Docker Engine on the Docker host. Do not install Docker Desktop and do not publish the raw socket. A `tcp://` endpoint outside Development needs the client certificate, client key, and CA as `file:/absolute/path` or `infisical:prod:/engine/client-cert` references on the host row. PostgreSQL stores those references and does not store PEM. Cleartext `tcp://` is rejected on registration and on connect. `unix` and `npipe` do not need that material. Development may keep cleartext `tcp://` for a nested or demo Engine. There is no separate lab insecure flag. See [ADR 0016](adr/0016-traefik-websecure-engine-mtls-and-socket.md). Check: the Engine answers on the management VLAN.
