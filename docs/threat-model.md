...
The `tcp://` certificate check is enforced on registration and on connect outside Development. References are `file:` or `infisical:` paths. PEM is not stored. The `websecure` TLS stamp is not enforced yet. The tenant socket denylist is already enforced.
