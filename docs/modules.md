...
| Delivery | `delivery` | Compose policy, deploy, rollback, worker lease, and the CI webhook. `POST /apps/{id}/approve` runs a deploy that was parked for approval. A failed Engine call is stored on the deployment as a short message that does not include secret values. |
