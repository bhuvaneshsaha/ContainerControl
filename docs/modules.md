...
| Delivery | `delivery` | Compose policy, deploy, rollback, a deploy lease per application and environment, and the CI webhook. A second deploy of the same application and environment returns 409 without waiting; the lease expires after five minutes if the holder does not release it. Different applications, or different environments of the same application id, deploy at the same time. Deploy injects a secret only into the services named on that secret, and fails when a service references a secret that is not assigned to it. Dependencies start first, and a service healthcheck is waited on before the next service. A team quota rejects a deploy that is missing CPU, memory, or storage limits or that exceeds the allowance. `POST /apps/{id}/approve` runs a deploy that was parked for approval. A failed Engine call is stored on the deployment as a short message that does not include secret values. See [ADR 0012](adr/0012-deploy-lease-per-application-environment.md). |
