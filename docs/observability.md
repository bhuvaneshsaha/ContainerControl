...
Publishing a template, removing one, and creating an application from one write a structured log with the template id and, for a create, the application id. The compose document and secret values are not logged. A rejected template logs that it was not published and does not log the document.
