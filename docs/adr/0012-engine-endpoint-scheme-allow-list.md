...
## Consequences
Operators register `unix:///var/run/docker.sock`, `npipe://./pipe/docker_engine`, or `tcp://host:port`. `http` and `https` addresses are not accepted. A rejected address is not stored. A connect that still carries a disallowed URI fails before a client is created. Client certificates for `tcp://` outside Development are [ADR 0016](0016-traefik-websecure-engine-mtls-and-socket.md): that record requires Infisical client material and rejects cleartext `tcp://` with a fixed message that does not include the address. `unix` and `npipe` stay as they are.
